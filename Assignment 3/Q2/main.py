...
if __name__ == '__main__':
    ...
    gtdict = dict()
    for i,j in gt:
        if i-1 in gtdict.keys():
            gtdict[i-1].append(j-1)
        else:
            gtdict[i-1] = [j-1]
    ...
    rf_sim_matrix = relevance_feedback(vec_docs, vec_queries, sim_matrix, gtdict)
    if args.verbose:
    ...
    rf_sim_matrix = relevance_feedback_exp(vec_docs, vec_queries, sim_matrix, tfidf_model, gtdict)
    if args.verbose:
        for i in range(rf_sim_matrix.shape[1]):
            ranked_documents = np.argsort(-rf_sim_matrix[:, i])
            print ('Query:', i+1, 'Top relevant 10 documents:', ranked_documents[:10] + 1)
